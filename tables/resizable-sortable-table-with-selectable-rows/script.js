...
function exportTableToCSV(table, { filename = 'transacciones.csv', selectedOnly = false, skipCheckboxCol = true } = {}) {
...
const columns = document.querySelectorAll('#team-table th.resizable')
...
columns.forEach((th, i) => {
  if (i < columns.length - 1) {
    ...
    resizer.style.background = '#cacacaff';
...
rowCheckboxes.forEach(checkbox => {
    checkbox.addEventListener('click', function() {
        ...
        selectAllCheckbox.checked = 
          allChecked.length === rowCheckboxes.length;
    });
});